...
class MDMetadataTest extends SapphireTest {
	...
	*/
	function testgetDateTimeNice()
	{
		...
		$metadata->loadData($item);
		//check initial value
		$this->assertEquals($metadata->MDDateTime,'2009-11-01 00:00:00','checking initial value');
		// convert to a nice data
		$date=$metadata->getDateTimeNice();
		$this->assertEquals($date,'01/11/2009', 'initial value');
		
		//checking a date in the other epoch
		$metadata->MDDateTime='1960-01-01 23:24:25';
		$date=$metadata->getDateTimeNice();
		$this->assertEquals($date,'01/01/1960' , 'date of other epoc');
		
		// checking empty value
		$metadata->MDDateTime='';
		$date=$metadata->getDateTimeNice();
		$this->assertEquals($date,MDCodeTypes::$default_for_null_value, 'empty date value');
		
		//null-Value should return the default for null
		$metadata->MDDateTime=null;
		$date=$metadata->getDateTimeNice();
		$this->assertEquals($date,MDCodeTypes::$default_for_null_value, 'null date value');
		
		//checking an empty ssDateTime value. (should be 1.1.1970 00:00:00)  
		$metadata->MDDateTime=new SS_Datetime();
		$date=$metadata->getDateTimeNice();

		$this->assertEquals($date,'01/01/1970 12:00pm', 'empty ssDateTime');
	}
	...
	*/
	function testgetDateTypeNice()
	{
		...
		$metadata->loadData($item);
		//check initial value
		$this->assertEquals($metadata->MDDateType,'publication','checking initial value');
		// convert to a nice Type
		$type=$metadata->getDateTypeNice();
	...
	*/
	function testgetTopicCategoryNice()
	{
		...
		$metadata->loadData($item);
		//check initial value
		$this->assertEquals($metadata->MDTopicCategory,'biota','initial data');
		
		// make it nice
		$topic=$metadata->getTopicCategoryNice();
		$this->assertEquals($topic,'Biota','on initial data');
		
		// checking empty value
		$metadata->MDTopicCategory='';
		$topic=$metadata->getTopicCategoryNice();
		$this->assertEquals($topic,MDCodeTypes::$default_for_null_value,'on empty value');
		
		//null-Value should return the default for null
		$metadata->MDTopicCategory=null;
		$topic=$metadata->getTopicCategoryNice();
		$this->assertEquals($topic,MDCodeTypes::$default_for_null_value,'on null value');
		
		//checking an invalid value
		$metadata->MDTopicCategory='INVALID_TOPIC';
		$topic=$metadata->getTopicCategoryNice();
