...
class CataloguePage_Controller extends Page_Controller {
	...
	static protected $xml_response = array (
		'iso'    => '../geonetwork/xslt/iso19139_to_silverstripe.xsl',
		'dublin' => '../geonetwork/xslt/dubliccore_to_silverstripe.xsl',
		'mcp' => '../geonetwork/xslt/mcp_to_silverstripe.xsl'
	);
	...
	static protected $xml_full_response = array (
		'iso'    => '../geonetwork/xslt/iso19139full_to_silverstripe.xsl',
		'mcp' => '../geonetwork/xslt/mcpfull_to_silverstripe.xsl'
	);
	...
	*/
	 ...
	 */
	protected function parseResponse($standard, $responseXML, $searchTerm) {

		// create command parameters
		$data = array(
			'xml' => $responseXML,
			'xsl' => self::$xml_response[$standard],
		);
	...
	static function validate_request_format($format){
		...
		if( isset($format) and isset(self::$xml_request[$format])){
			return $format;
		}
	 ...
	 * This method is used to delegate a search request to the geonetwork catalogue 
	 ...
	 *
	 * @param array $data request parameters. Requires $data['searchterm']
	 *
	...
	public function dogetrecords( $params ) {
		...
		$data = array(
			'searchterm' => Convert::raw2xml($searchTerm),
			'requestxml' => self::$xml_request[$format],
